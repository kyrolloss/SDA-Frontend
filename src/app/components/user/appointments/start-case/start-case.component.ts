...
export class StartCaseComponent implements OnInit,OnDestroy {
  ...
  { key: 'inflammation', label: 'Inflammation (red, painful)', selected: false },
  { key: 'lesions', label: 'Lesions (aphthous ulcers, herpes)', selected: false },
...
gingivalColorOptions = [
  { key: 'healthy', label: 'Coral pink, stippled (Healthy)', selected: false },
  { key: 'acute', label: 'Red, shiny (Acute gingivitis)', selected: false },
  { key: 'chronic', label: 'Bluish, swollen (Chronic gingivitis)', selected: false },
  { key: 'pigmented', label: 'Pigmented (Melanin pigmentation)', selected: false },
];
...
bleedingOnProbingOptions = [
  ...
  { key: 'present', label: 'Present on probing (Gingivitis)', selected: false },
...
occlusionClassOptions = [
  ...
  { key: 'class2', label: 'Class II (Division 1 / Division 2)', selected: false },
...
toothMobilityOptions = [
  ...
  { key: 'grade3', label: 'Grade III (>1 mm, horizontal + vertical)', selected: false },
];

cariesOptions = [
  { key: 'code0', label: 'Code 0: Sound', selected: false },
  { key: 'code1_2', label: 'Code 1–2: Enamel change only', selected: false },
  { key: 'code3_4', label: 'Code 3–4: Enamel breakdown or dentin shadow', selected: false },
  { key: 'code5_6', label: 'Code 5–6: Obvious cavity with dentin exposure', selected: false },
];
...
pulpVitalityOptions = [
  ...
  { key: 'prolonged', label: 'Prolonged pain (Irreversible pulpitis)', selected: false },
  { key: 'noresponse', label: 'No response (Pulp necrosis)', selected: false },
...
toothDiagnosisOptions = [
  ...
  { key: 'periodontitis', label: 'Acute apical periodontitis', selected: false },
...
periodontalDiagnosisOptions = [
  ...
  { key: 'periodontitis', label: 'Periodontitis (Stage I–IV, Grade A–C)', selected: false },
];
...
treatmentPlanOptions = [
  ...
  { key: 'disease', label: 'Disease control phase (Fillings, Periodontal therapy)', selected: false },
  { key: 'definitive', label: 'Definitive care (Crowns, Bridges, Prosthetics)', selected: false },
  { key: 'maintenance', label: 'Maintenance / Preventive phase', selected: false },
];
  ...
  private sectionMap = [
    ...
    { keywords: ['relevant medical history', 'medical history', 'history'], handler: 'analyzeSpeechForMedicalHistory' },
    ...
    { keywords: ['tongue', 'soft tissue', 'tissues'], handler: 'analyzeSpeechForSoftTissue' },
{ keywords: ['gingival color', 'color texture', 'gingiva color'], handler: 'analyzeSpeechForGingivalColor' },
{ keywords: ['gingival enlargement', 'enlargement'], handler: 'analyzeSpeechForGingivalEnlargement' },
{ keywords: ['bleeding on probing', 'bop', 'bleeding probing'], handler: 'analyzeSpeechForBleedingOnProbing' },
{ keywords: ['pocket depth', 'probing depth', 'pd'], handler: 'analyzeSpeechForPocketDepth' },
{ keywords: ['angle classification', 'occlusion class', 'class one', 'class two', 'class three'], handler: 'analyzeSpeechForOcclusionClass' },
{ keywords: ['vertical overlap', 'overbite'], handler: 'analyzeSpeechForVerticalOverlap' },
{ keywords: ['tooth mobility', 'mobility'], handler: 'analyzeSpeechForToothMobility' },
{ keywords: ['caries', 'icdas'], handler: 'analyzeSpeechForCaries' },
{ keywords: ['existing restoration', 'restoration', 'fillings'], handler: 'analyzeSpeechForExistingRestorations' },
{ keywords: ['pulp vitality', 'cold test', 'vitality'], handler: 'analyzeSpeechForPulpVitality' },
{ keywords: ['tooth diagnosis', 'pulpitis', 'apical', 'abscess'], handler: 'analyzeSpeechForToothDiagnosis' },
{ keywords: ['periodontal diagnosis', 'periodontitis', 'gingivitis', 'gum disease'], handler: 'analyzeSpeechForPeriodontalDiagnosis' },
{ keywords: ['caries risk', 'risk assessment'], handler: 'analyzeSpeechForCariesRisk' },
{ keywords: ['prognosis'], handler: 'analyzeSpeechForPrognosis' },
{ keywords: ['treatment plan', 'basic plan', 'phase'], handler: 'analyzeSpeechForTreatmentPlan' },
  ];
  ...
  async startRecording() {
    ...
    try {
      ...
      this.mediaRecorder.onstop = () => {
        this.downloadFileName = `recording_${new Date().toISOString().replace(/[:.]/g, '-')}.mp4`;
...
private analyzeSpeechWithContext(transcript: string) {
  ...
  for (let i = 0; i < foundSections.length; i++) {
    ...
    if (typeof fn === 'function' && sectionText) {
      console.log(`🧠 Analyzing section: ${this.currentSection} →`, sectionText);
...
analyzeSpeechForVerticalOverlap(transcript: string) {
  const lower = transcript.toLowerCase();
  const map: Record<string, string[]> = {
    normal: ['normal', 'two millimeters', 'three millimeters', 'four millimeters'],
...
analyzeSpeechForToothMobility(transcript: string) {
  ...
  const map: Record<string, string[]> = {
    ...
    grade3: ['grade 3', 'severe mobility', 'vertical movement', 'highly mobile'],
  };
...
analyzeSpeechForToothDiagnosis(transcript: string) {
  const lower = transcript.toLowerCase();
  const map: Record<string, string[]> = {
    reversible: ['reversible pulpitis', 'mild pain', 'short response', 'temporary pain'],
    irreversible: ['irreversible pulpitis', 'prolonged pain', 'lingering', 'severe pain'],
    necrosis: ['pulp necrosis', 'dead tooth', 'no response', 'non vital'],
    periodontitis: ['apical periodontitis', 'periapical', 'tender tooth', 'biting pain'],
...
analyzeSpeechForPeriodontalDiagnosis(transcript: string) {
  ...
  const map: Record<string, string[]> = {
    ...
    periodontitis: ['periodontitis', 'bone loss', 'deep pockets', 'advanced gum disease'],
  };
...
analyzeSpeechForTreatmentPlan(transcript: string) {
  ...
  const map: Record<string, string[]> = {
    ...
    disease: ['disease control', 'fillings', 'periodontal therapy', 'scaling'],
    definitive: ['definitive care', 'crowns', 'bridges', 'prosthetics', 'final restoration'],
    maintenance: ['maintenance', 'follow up', 'preventive phase', 'recall visit'],
  };

...
export class ClinicsComponent implements OnInit {
  ...
  get data(): any {
    ...
    if (response?.ownerClinics?.data?.length) {
      response.ownerClinics.data = response.ownerClinics.data.map(
        (clinic: any, index: any) => {
          if (index === 0) {
    ...
    if (response?.subOwnerClinics?.data?.length) {
      response.subOwnerClinics.data = response.subOwnerClinics.data.map(
        (clinic: any, index: any) => {
          if (index === 0) {
            // 🟢 Premium package — everything unlimited
            clinic.features = {
    ...
    if (response?.operatorClinics?.data?.length) {
      response.operatorClinics.data = response.operatorClinics.data.map(
        (clinic: any, index: any) => {
          if (index === 0) {
            // 🟢 Premium package — everything unlimited
            clinic.features = {
  ...
  openEditModal(clinic: any) {
    ...
    this.clinicsService.getClinicById(clinic.id).subscribe({
      next: (data) => {
      ...
      error: () => {
        this._MatSnackBar.open('Failed to load clinic data', 'Close', {
          duration: 3000,
          panelClass: ['snackbar-error'],
        });
      },
  ...
  closeCreateClinic() {
    ...
    this.isAutoGeocodeEnabled = true;
    this.form.reset();
    this.lat = null;
    this.lng = null;
    this.showMap = false;
